  ...
  <v-container fluid>
    ...
    <v-card v-for="host in hosts" :key="host.id">
        ...
        <v-card-text v-if="host.monitoring_url">
            ...
            <iframe :src="`${host.monitoring_url}&kiosk&theme=${$vuetify.theme.dark ? 'dark' : 'light'}`"
                @load="loaded(host)" v-show="host.monitoringLoaded" class="grafana" referrerpolicy="no-referrer" />
        </v-card-text>
    </v-card>
  </v-container>
</template>
